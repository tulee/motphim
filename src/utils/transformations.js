...
export const genreTopVideoTransformation = async (genres, apiCallType) => {
    let url
    if (apiCallType === 'tv') {
        url = `discover/tv?api_key=${apiKey}&language=en-US&sort_by=popularity.desc&page=1&include_null_first_air_dates=false&with_genres=`
    } else if (apiCallType === 'movie') {
        url = `discover/movie?api_key=${apiKey}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=`
    }
...
export const mediaTypeToVideoDetailTransformation = async (videoId, mediaType) => {
    let requestURL;
    if (mediaType === 'movie') {
        requestURL = `movie/${videoId}?api_key=${apiKey}&language=en-US`
    } else if (mediaType === 'tv') {
        requestURL = `tv/${videoId}?api_key=${apiKey}&language=en-US`
    }
